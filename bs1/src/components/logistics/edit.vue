		...
		data() {
			...
			methods: {
				...
				onSubmit() {
					...
					var params=`wnum=${this.form.wnum}&snote=${this.form.snote}&spos=${this.form.hname+this.form.rnum}&sid=${this.$store.state.sid}`;
					fetch("/api/service/edit?"+params).then(function(e){
						return e.text();
					}).then((e)=>{
						if(e=="ok"){
							this.$message({
			...
			mounted() {
				...
				fetch("/api/service/selBySid?sid="+this.$store.state.sid).then(function(e){
					return e.json();
				}).then((e) => {
					var arr=e[0].spos.split("栋");
					this.form.wnum=e[0].wnum.toString();
					this.form.snote=e[0].snote;
					this.form.hname=arr[0]+"栋";
					this.form.rnum=arr[1];
				});
			}
	}
</script>
